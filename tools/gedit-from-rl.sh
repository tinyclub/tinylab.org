...
# Remove mermaid pics and let the generate png come out
sed -i -e '/^\s*```mermaid/,/^\s*```/d' $_target_article
sed -i -e 's/（\[下载由 Mermaid 生成的 PNG 图片\]\[\(.*\)\]）/![此图由 Mermaid 生成][\1]/g' $_target_article

# article ads
article_ads="学习 RISC-V Linux，就用泰晓社区自研即插即跑 RISC-V Linux 实验盘（https://tinylab.org/linux-lab-disk），已集成 RISC-V Linux 实验手册 v0.2，某宝检索 “泰晓 Linux” 即可找到。**Linux Lab v1.2 Inside —— 内核开发从未像今天这般简单！**"

# article info
[ -z "$key" ] && key="$(echo $info | sed -e 's/riscv/RISC-V/g;s/sbi/SBI/g;s/\(.*\)part.*/\1/g')"
key="$(echo $key | tr '[A-Z]' '[a-z]' | sed -e 's/\([ ]\)*\([a-z\]\)\([^ ]*\)/\1\U\2\L\3/g')"
article_info="本周继续连载 $key 系列文章，记得收藏分享+关注，合集：https://tinylab.org/riscv-linux"
...
top_title_line=$(grep -n -m1 '^# ' $_target_article | cut -d ':' -f1)

sed -i -e ''${top_title_line}'i\\n' $_target_article
sed -i -e "${top_title_line}i$article_ads" $_target_article
sed -i -e ''${top_title_line}'i\\n' $_target_article
sed -i -e "${top_title_line}i$article_info" $_target_article
