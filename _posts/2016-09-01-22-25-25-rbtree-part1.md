...
由于 rb_subtree_gap 由 vma_compute_subtree_gap 计算得，单纯从计算角度，vma_compute_subtree_gap 是“递归”调用自己，这个“递归”在“叶子节点” T 和 R 处解套：
...
代入图例场景，这是个中序遍历（In-Order Traversal，即从低到高扫描）过程，其中 rb_subtree_gap 用来过滤路径上“名下空隙太小”的节点：

	# 前提假定：待分配的空间，要么可在 P 名下找到；要么没有符合要求的空闲空间
	# 伪代码描述，找 P 名下的满足分配要求、地址最低的空隙
	#
	# 出场变量简介：
	#   length: 分配请求的长队，对齐到对齐要求
