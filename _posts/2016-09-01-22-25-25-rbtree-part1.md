...
由于 rb_subtree_gap 由 vma_compute_subtree_gap 计算得，单纯从计算角度，vma_compute_subtree_gap 是“递归”调用自己，这个“递归”在叶子节点 T 和 R 处解套：
...
<p><span style="color:#a6aaa9">要知道程序的某个特性，最好看它是怎么用的。</span>下面从 <a href="http://lxr.free-electrons.com/source/mm/mmap.c?v=4.7#L1585">unmapped_area()</a>，来看 rb_subtree_gap 是如何发挥作用的？</p>
	...
	// http://lxr.free-electrons.com/source/mm/mmap.c?v=4.7#L1585
	// 分配未使用的虚拟地址段，info 指出了分配要求
	unsigned long unmapped_area(struct vm_unmapped_area_info *info)
	{
		...
		length = info->length + info->align_mask;
		...
		// 分配地址段允许的`最大开始地址'，gap_start 应小等于之
		high_limit = info->high_limit - length;
		...
		// 分配地址段允许的`最小结束地址'，gap_end 应大等于之
		low_limit = info->low_limit + length;
		...
	
		// 在空隙中找
		//（若找不到，从最高处开始找（图例中 R 之后的空间））
		while (true) {
			/* Visit left subtree if it looks promising */
			// 先尝试左子节点，似是低地址优先原则
			gap_end = vma->vm_start;
			// 对应到图例：gap_end 是以 P 的视角得来的,
	// | G1 |  L  | G2 | T | G3 | P |...
	// |----|-----|----|---|----|---|
			// 即 gap_end = max(G1.end，G2.end, G3.end)

			// 而从左子节点 L 的视角而言，仅能看到 G1 和 G2
			// 即 gap_end = max(G1.end，G2.end)
			// 故可能存在进入左子节点，寻找无果
			// 这就是为什么需在循环的最后：
			...
			// vma 名下空隙足够大，但其左子节点名下的空隙不够大（或不满足 gap_end >= low_limit 要求）
			// 根据 vma_compute_subtree_gap 计算规则，只可能：
			// 1. 紧挨着 vma 的空隙够大
			// 2. vma 的右子节点名下的空隙够大
			// 按照低地址优先的原则，先检查 1
	...
	check_current:
			...
			// 如果紧挨着 vma 的空隙，其开始地址还嫌太高了
			//（超过 high_limit），由于接下来要查右子节点，
			// 空隙地址肯定更高。没戏了，返回无内存
