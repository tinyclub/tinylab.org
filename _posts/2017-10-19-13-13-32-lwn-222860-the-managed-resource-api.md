...
在一月份我们讨论了[设备资源管理补丁](http://lwn.net/Articles/215996/)。该补丁现在已经合入内核版本 2.6.21 。鉴于该套 API 已经成为了所有内核正式 API 的一部分 - 是时候好好给大家介绍一下这套新接口了。
...
提供这套资源管理接口的基本出发点是解决适时释放资源的难题。这是个一个令广大驱动开发人员倍感头痛的问题，驱动开发人员也因此常常被指责给内核引入了太多的 bug，虽然在这里本文暂且不去探讨这种指责是否有失公允。即使是最好的驱动开发人员，如果遇到设备探测中途失败的情况也可能会感到十分棘手；代码中也会考虑失败后释放资源的操作，但常常由于条件限制很难被充分地测试。所有的这些问题都会导致驱动程序代码中产生资源泄漏。
...
为了解决这个问题，Tejun Heo 提出了一套新的资源分配函数，可用于跟踪驱动代码中的资源分配。所有的资源分配都和对应的设备结构体相关联; 当驱动程序与设备分离时，曾经分配给该设备的任何不再使用的资源都将被释放归还。因此，这些资源管理调用接口非常类似于 Samba 系统中提供的 [talloc() API](http://samba.org/ftp/unpacked/samba4/source/lib/talloc/talloc_guide.txt) ，但更适用于内核环境，并且覆盖的不仅仅是内存资源分配。
...
这些新的函数的定义方式和后面介绍的其他函数是类似的，内存分配的资源管理函数除了在函数名称上增加了前缀以及添加了一个 `dev` 参数外，其他部分看上去和对应的 `kzalloc()` 和 `kfree()` 函数几乎一模一样。新增加的 `dev` 参数用于辅助资源管理模块的代码判断何时可以释放内存。如果在该设备被移除的时候其关联的任何内存仍未被释放，那么资源管理模块会检测出来并在此时自动完成释放操作。
...
值得注意的是，没有提供对应于 `kalloc()` 的资源管理函数版本 ; 理由是如果在释放内存问题上驱动开发人员不能被信任的话，那么在初始化内存这个问题上他们同样不能被信任（译者注，这里 `kalloc()` 应该是 `kmalloc()` 的笔误，另外 `kzalloc()` 和 `kmalloc()` 的主要区别是 `kzalloc()` 会在 `kmalloc()` 的基础上确保将申请的内存全部初始化为零，所以这里猜测内核不提供形如 `devm_kmalloc()` 这样的接口可能是出于一种推荐使用上的考虑，也就是鼓励大家多用形如 `kzalloc()` 这样封装得更好的接口 ）。同样地也没有提供页面级别内存分配和 slab 分配函数的资源可管理版本。

> Managed versions of a subset of the DMA allocation functions have been provided:

对应于 DMA 分配功能的资源管理版本形式如下：
...
所有这些函数都具有与其对应的 `dma_*` 函数相同的参数和类似功能 ，但是在此基础上会确保设备关闭时释放申请的 DMA 内存。注意资源管理模块只保证 DMA 内存会被释放，作为驱动代码来说依然要确保不要在 DMA 内存释放后还会去访问这些内存区域，否则会造成无法预期的问题。
...
针对 I/O 内存管理，设备资源管理补丁直接修改了现有的 `pci_request_region()` 函数的内部实现增加了相应的资源管理功能，而不是提供形如带有 `pcim_` 前缀的对应版本。但补丁对更高层次的分配和映射函数接口提供了资源管理版本，如下：
...
同样注意处于资源管理的目的这些函数都新增了一个 `struct device` 的参数 `dev`。
...
组可以用来标识与给定设备相关联的一组资源分配序列。我们可以使用 `devres_open_group()` 创建一个组并标识此后发生的资源分配都属于该组，调用该函数时可以通过参数 `id` 传入一个组的标识或者直接给空值 NULL 则该函数会自动生成该标识; 无论哪种方式，新建的组的标识都会通过函数的返回值返回。调用 `devres_close_group()` 可以标记给定组的结尾。调用 `devres_remove_group()` 会删除当前组，但不会释放该组中分配的资源。要删除组并立即释放该组中分配的所有资源， 应使用另一个函数 `devres_release_group()`。
...
组函数看上去主要是供内核中的那些中间层组件 - 例如总线层 - 调用。例如，当总线代码尝试将驱动程序关联到一个设备时，它可以新建一个组; 如果驱动程序关联失败，内核可以利用该组记录的信息将驱动程序分配的任何资源全部释放掉。
