...
在过去，Linux 的使用者经常需要告诉内核具体设备的位置。缺少这些信息的话，驱动无从得知设备的 I/O 端口和中断线的信息从而无法配置和使用设备。幸运的是，现在存在像 PCI 这样的总线设备具备内建的自动发现设备的功能，PCI 总线上的设备可以通过总线告诉系统设备的类型以及其接入系统时需要配置的资源信息。所以内核可以在系统引导阶段就枚举这些设备。
...
可惜，生活并不总是那么简单；事实上依然存在大量的设备不可以被处理器检测到。在嵌入式和偏上系统的世界里，不能够在启动阶段被检测的设备正变得越来越多。所以我们仍然需要提供一种方法来告诉内核这些硬件设备的连接情况。为此内核开发创建了 "平台设备" ("Platform devices") 来代表这一类硬件设备。本文将描述平台设备的调用接口；这些也是继续[后继文章](https://lwn.net/Articles/448502/)所必须的背景知识。
...
内核定义结构体 `struct platform_device` 来表示一个平台设备，该定义和本文其他相关的代码都定义在 `<linux/platform_device.h>` 中。平台设备假象为连接在一个虚拟的“平台总线”上；对应的平台驱动负责将其注册到平台总线上。注册的行为需要通过定义一个 `platform_driver` 结构体来完成。
...
定义平台驱动时至少需要定义 `probe()` 和 `remove()` 这两个回调函数；其他的回调函数和电源管理有关，需要在涉及到时提供。
...
驱动还需要做的一件事情就是供总线调用的方法用于将实际的设备和该驱动绑定起来；目前有两种机制实现该功能。第一种和 `struct platform_driver` 结构体中的 `id_table` 成员有关，其类型定义如下：
...
如果驱动提供了该 ID 表，总线的代码在为新发现的平台设备搜索对应驱动时会检查系统已知的所有驱动的该字段。一旦发现设备的名字和某个驱动的 ID 表的 `name` 字段匹配， 则该设备就和该驱动实现绑定；同时驱动得到一个指向该 ID 表的指针。但在实际代码实现中，大部分的平台驱动并没有提供 ID 表；而是为 `struct platform_driver` 的 `driver` 成员提供了一个名字。下面是一个实际的例子，i2c-gpio 驱动利用 GPIO 的两个管脚实现了 I2C 功能总线；为此其定义了一个平台驱动如下:
...
基于该驱动，所有名字包含 "i2c-gpio" 的设备会被绑定在该驱动上，使用该方法后就不用定义 ID 表了。
...
驱动实现 `probe()` 中一旦发现其所需的所有信息都必备后，应该尽可能地验证设备存在的合法性，然后将真实的设备和平台设备关联起来，成功情况下该回调函数必须返回 0。
...
现在我们有了一个平台驱动，太还没有定义实际的设备。正如前面所述，平台设备理论上是不可以自动被检测到的，所以必须采用另外一种方法让内核能够意识到其存在。典型地我们可以编码定义一个静态的 `platform_device` 类型变量，并至少提供一个名字以便内核为其绑定驱动。下面是一个假想的简单的设备，定义如下：
...
以上信息对于实例化一个简单的平台设备应该是足够的了，但实际的设备比它要复杂得多。甚至假想的 i2c-gpio 驱动也需要补充更多的信息才可以工作，譬如实际的 GPIO 线信息，用于实现 i2c 的时钟线和数据线。用于注册这些信息的数据结构称之为“平台数据”；该数据结构通过 `platform_device` 结构体的 `dev.platform_data` 传给驱动。
...
并不是内核社区的所有人都喜欢平台设备；采用该机制类似于在内核中硬编码硬件的信息。此外平台数据机制缺乏类型检查，驱动只能简单假设它们获取的数据类型是它们所期望的。即便如此，平台设备在内核中被大量使用，部分创建和发现机制有所改进，但大的现状不太会被改变。未来将交给设备树，将在后继文章中给大家介绍。